...
function locateUmdModule(astExpression) {
...
exports.default = locateUmdModule;
;
...
function isUmdTest(expression) {
    return expression && expression.type === 'LogicalExpression'
        ...
        && (isTypeofFunction(expression.left)
            || isSecondLevelUmdTest(expression.left)) && isDefineAmd(expression.right);
}
...
function isTypeofFunction(expression) {
    return expression && expression.type === 'BinaryExpression'
        && (expression.operator === '==='
            || expression.operator === '==') && ((isTypeofDefine(expression.left) && isFunctionLiteral(expression.right))
...
function containsDefineStatement(statement) {
    return statement && ((statement.type === 'BlockStatement' && first(statement.body, isDefineStatement))
...
//# sourceMappingURL=locateUmdModule.js.map